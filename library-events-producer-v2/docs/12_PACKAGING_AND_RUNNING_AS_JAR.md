...
* [Packaging and Running the App as a JAR File](#packaging-and-running-the-app-as-a-jar-file)
  * [Background: Running from IntelliJ vs. Running a JAR](#background-running-from-intellij-vs-running-a-jar)
  * [What Is a Spring Boot Executable JAR?](#what-is-a-spring-boot-executable-jar)
...
## Background: Running from IntelliJ vs. Running a JAR

So far we have been launching `LibraryEventsProducerV2Application` directly from IntelliJ by clicking the **Run** button. Here is why we need to move beyond that:

- **Production apps never run in an IDE.** Production environments are Linux servers, Docker containers, Kubernetes pods, or cloud VMs — none of which have IntelliJ installed.
- **IntelliJ run is tied to your local machine.** It depends on your IDE configuration and cannot be handed off to anyone or anything else.
- **It cannot be automated.** A CI/CD pipeline cannot click the Run button — it needs a deployable artifact.
- **A JAR is that artifact.** It is a single, self-contained file that runs identically on any machine with Java installed.
- **It is the foundation for everything that follows** — Docker images, Kubernetes deployments, and automated release pipelines all start with a JAR.

---

## What Is a Spring Boot Executable JAR?

The Spring Boot Gradle plugin (`org.springframework.boot`) repackages the standard compiled output into a **fat JAR** (also called an **uber JAR** or **executable JAR**). This single file contains:

- Your compiled application classes
- All dependency JARs (Kafka client, Jackson, Spring MVC, Actuator, etc.)
- An embedded web server (Tomcat by default)
- A Spring Boot launcher that boots everything up

You only need a JDK/JRE on the target machine — nothing else.
...
```
build/libs/library-events-producer-v2-0.0.1-SNAPSHOT.jar
```
...
```
-rw-r--r--  1 user  staff  XX MB  library-events-producer-v2-0.0.1-SNAPSHOT.jar
```
...
Make sure Kafka is running (e.g., via Docker Compose) before starting the app.
...
- `SPRING_PROFILES_ACTIVE` — maps to `spring.profiles.active`
- `SPRING_KAFKA_BOOTSTRAP_SERVERS` — maps to `spring.kafka.bootstrap-servers`
...
Once you see the Spring Boot banner and the log line `Started LibraryEventsProducerV2Application`, the app is up.
...
`spring-boot-starter-actuator` is already a dependency. When the app runs as a JAR, the `/actuator/health` and `/actuator/info` endpoints are available immediately — no IDE plugin needed. These are the same endpoints Kubernetes liveness/readiness probes call.
